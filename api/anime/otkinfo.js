...
async function otakuInfo(links) {
  try {
    ...
    let result = {
      ...
      episode: [
        { url: urls[0]?.url || '', eps: urls[0]?.eps || '', rilis: uploads[0]?.upload || '' },
        { url: urls[1]?.url || '', eps: urls[1]?.eps || '', rilis: uploads[1]?.upload || '' },
        { url: urls[2]?.url || '', eps: urls[2]?.eps || '', rilis: uploads[2]?.upload || '' },
        { url: urls[3]?.url || '', eps: urls[3]?.eps || '', rilis: uploads[3]?.upload || '' }
      ]
    };
...
module.exports = {
  ...
  async run(req, res) {
    ...
    try {
      ...
      res.status(500).json({ status: false, error: error.message || 'Terjadi kesalahan saat mengambil data' });
    }
  }
};