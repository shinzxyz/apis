...
module.exports = {
  ...
  category: 'Search',
  params: ['query', 'count'], // count opsional
  async run(req, res) {
    ...
    try {
      ...
      const search = await axios.get(
        `https://api.spotify.com/v1/search?q=${encodeURIComponent(query)}&type=track&limit=${count || 5}`,
        {
      ...
      if (!tracks.length) return res.status(404).json({ status: false, error: "Tidak ditemukan hasil." });
