...
module.exports = {
  ...
  async run(req, res) {
    ...
    // pastikan count adalah angka, default 10, max 50
    let limit = 10;
    if (count) {
...
async function caripinterest(query, limit) {
  try {
    ...
    try {
      ...
      return response.result.slice(0, limit).map(item => ({ link: item.link, directLink: item.imageUrl }));
    } catch {
      try {
        ...
        return res.slice(0, limit).map(item => ({ link: item.link, directLink: item.directLink }));
      } catch {
        try {
          ...
          return res.result.slice(0, limit).map(item => ({ link: item.link, directLink: item.directLink }));
        } catch {
          try {
            ...
            return res.results.slice(0, limit).map(item => ({ link: item.source, directLink: item.image }));
          } catch {
            try {
              ...
              return res.result.slice(0, limit).map(url => ({ link: 'https://www.pinterest.com', directLink: url }));
            } catch {
              try {
                ...
                return res.result.pins.slice(0, limit).map(pin => ({ link: pin.pin_url, directLink: pin.media?.images?.orig?.url }));
              } catch (err) {
