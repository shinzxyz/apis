...
async function spotifySearch(query, limit = 10, offset = 0, market = 'id') {
  ...
  if (!token) return { status: false, message: 'Gagal mendapatkan access token' };
  ...
  try {
    ...
    if (!items.length) return { status: false, message: `${query} tidak ditemukan` };
    ...
    return { status: false, message: 'Gagal memuat data dari Spotify' };
  }
}
