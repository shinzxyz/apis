...
module.exports = {
  ...
  async run(req, res) {
    ...
    try {
      ...
      // Jika bukan URL langsung, lakukan pencarian ke Spotify API
      if (!isSpotifyUrl) {
        ...
        const searchSpotify = await axios.get(
      ...
      // Langkah 1: Ambil ID lagu dari fabdl
      ...
      const { id, gid, name, artists, duration_ms, image } = info.data.result;

      // Langkah 2: Konversi ke MP3
      ...
      const file = await axios.get(mp3Url, { responseType: 'arraybuffer' });
