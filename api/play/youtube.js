...
const { download } = require('../../lib/savetube');
...
module.exports = {
  ...
  async run(req, res) {
    ...
    try {
      ...
      if (!saveResult.download) return res.status(500).json({ status: false, error: 'Gagal mendapatkan link download MP3' });

      const audioRes = await axios.get(saveResult.download, { responseType: 'arraybuffer' });
