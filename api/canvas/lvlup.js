...
module.exports = {
  ...
  async run(req, res) {
    const { avatar, background, username, avatarBorderColor, overlayOpacity, currentLevel, nextLevel } = req.query;

    if (!avatar || !background || !username || !avatarBorderColor || !overlayOpacity || !currentLevel || !nextLevel)
