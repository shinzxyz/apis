...
module.exports = {
  name: 'Welcome', // ubah ke goodbye, promote, demote sesuai file
  desc: 'Generate kartu sambutan',
  category: 'Canvas',
  params: ['background', 'description', 'member', 'avatar'],
  async run(req, res) {
    const { background, description, member, avatar } = req.query;
    ...
    if (!background || !description || !member || !avatar)
      return res.status(400).json({ status: false, error: 'Parameter background, description, member, avatar wajib diisi!' });
