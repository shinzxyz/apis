...
async function request(url) {
  const { token_v, token_c, token_h } = await getToken();
  const data = qs.stringify({ URLz: url, token_v, token_c, token_h });
  ...
  async run(req, res) {
    ...
    try {
      ...
      if (!result.normal && !result.hd) {
        return res.status(404).json({ status: false, message: 'Gagal mendapatkan link video.' });
      }
      res.json({ status: true, ...result });
    } catch (e) {
      res.status(500).json({ status: false, message: 'Gagal memproses video.', error: e.message });
    }
  }
};