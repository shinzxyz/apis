...
const { default: fetchJson } = require("../../lib/fetcher");
...
module.exports = {
  ...
  async run(req, res) {
    ...
    try {
      // Cadangan 2 - pinterestdownloader.io
      ...
      return res.status(500).json({ status: false, error: "Gagal mengambil media dari Pinterest." });
    }
  },
};
...
async function pintod(url) {
  try {
    ...
    if (!medias.length) return { error: "Media tidak ditemukan" };
