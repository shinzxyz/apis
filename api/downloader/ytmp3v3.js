...
module.exports = {
  ...
  async run(req, res) {
    ...
    try {
      ...
      if (!infoRes.data.status) return res.status(404).json({ status: false, error: 'Video tidak ditemukan' });

      const data = infoRes.data.data;
      const { status, title, thumbnail, durationLabel, id, duration, key } = data;

      // Ambil info video dari YouTube API
      ...
      if (!mp3Url) return res.status(500).json({ status: false, error: 'Gagal mendapatkan link download MP3' });
      ...
      console.error(e);
      return res.status(500).json({ status: false, error: e.message || 'Terjadi kesalahan' });
    }
  }
}
