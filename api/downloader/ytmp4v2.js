  ...
  async run(req, res) {
    ...
    try {
      ...
      if (resInfo.data.status !== true) {
        return res.status(400).json({ status: false, error: 'Gagal mengambil data video' });
      }

      const data = resInfo.data.data;
      const { key, status, title, thumbnail, durationLabel, id, duration } = data;

      // Ambil detail video YouTube lewat API Google
      ...
      res.status(500).json({ status: false, error: e.message || 'Terjadi kesalahan' });
    }
  }
};
