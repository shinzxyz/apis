...
module.exports = {
    ...
    async run(req, res) {
        ...
        try {
            ...
            if (!mediaInfo.downloadUrl) return res.status(404).json({ status: false, message: 'Media tidak ditemukan.' });
            ...
            res.status(500).json({ status: false, message: 'Gagal memproses permintaan.', error: err.message });
        }
    }
}