...
module.exports = {
  ...
  async run(req, res) {
    const { url } = req.query;
    if (!url) return res.status(400).json({ status: false, error: 'Parameter url dibutuhkan' });
    ...
    try {
      const resSearch = await axios('https://x2download.app/api/ajaxSearch', {
        method: 'POST',
        data: new URLSearchParams({ q: url, vt: 'home' }).toString(),
        headers: {
          'Accept': '*/*',
      ...
      const { vid: id, token, timeExpires: timeExpire, fn: fname, title } = resSearch.data;
      ...
      if (!items || items.length === 0) {
        return res.status(400).json({ status: false, error: 'Video tidak ditemukan di YouTube' });
      }
      ...
      while (code !== 200) {
        responseConvert = await axios.post(baseUrl, new URLSearchParams(data).toString(), { headers });
        ...
        if (code === 200) {
          ...
          return res.status(500).json({ status: false, error: 'Gagal konversi video' });
        }
      }

    } catch (error) {
      return res.status(500).json({ status: false, error: error.message || 'Terjadi kesalahan' });
    }
  }
};
