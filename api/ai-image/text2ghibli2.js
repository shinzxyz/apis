...
module.exports = {
  ...
  async run(req, res) {
    ...
    const endpoints = [
      'https://api.nekorinn.my.id/ai-img/text2ghibli-v2?text='
    ];
    ...
    for (const base of endpoints) {
      try {
        ...
        continue; // lanjut ke endpoint berikutnya jika gagal
      }
    }

    res.status(500).json({ status: false, error: 'Semua API gagal merespons.' });
  }
};