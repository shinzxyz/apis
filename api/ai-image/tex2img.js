...
module.exports = {
  ...
  async run(req, res) {
    ...
    if (isNaN(model) || model < 0 || model >= LIST_MODEL.length) {
      return res.status(400).json({ status: false, error: `Model tidak valid. Pilih salah satu: ${LIST_MODEL.map((m, i) => `${i} = ${m}`).join(', ')}` });
    }

    try {
      const { data } = await axios.post(BASE_URL + GENERATE_URL, payload(prompt, model), {
        headers: {
      ...
      return res.status(500).json({ status: false, error: e.message || e.toString() });
    }
  }
};