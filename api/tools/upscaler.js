...
module.exports = {
  ...
  async run(req, res) {
    ...
    try {
      ...
      if (!code) return res.status(500).json({ status: false, error: 'Gagal mendapatkan kode hasil!' });
      ...
      if (statusCheck.data.code === 200) {
        await new Promise(resolve => setTimeout(resolve, 5000)); // Delay sedikit sebelum hasil tersedia
        ...
        return res.status(500).json({ status: false, error: 'Upscaling gagal atau masih diproses.' });
      }

    } catch (err) {
      console.error('Upscaler Error:', err);
      return res.status(500).json({ status: false, error: err.message || err.toString() });
    }
  }
};