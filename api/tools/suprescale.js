...
module.exports = {
  ...
  async run(req, res) {
    const { url, resize, anime } = req.query;

    if (!url || !resize || !anime)
      return res.status(400).json({ status: false, error: 'Parameter url, resize, dan anime wajib diisi!' });

    const validResize = ['2', '4', '6', '8', '16'];
    const isAnime = anime === 'true' || anime === 'false';

    if (!validResize.includes(resize) || !isAnime)
      return res.status(400).json({ status: false, error: 'resize hanya boleh 2, 4, 6, 8, atau 16 dan anime harus true/false' });
