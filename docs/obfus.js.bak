document.addEventListener('DOMContentLoaded', function() {
    ...
    // Obfuscate button click handler
    obfuscateBtn.addEventListener('click', async function() {
        ...
        try {
            // Use the client-side obfuscator directly
    ...
    copyBtn.addEventListener('click', function() {
        navigator.clipboard.writeText(obfuscatedCode.textContent)
            .then(() => showStatus('Copied to clipboard!', 'green'))
            .catch(err => showStatus('Failed to copy', 'red'));
    });

    // Download button click handler
    downloadBtn.addEventListener('click', function() {
        const blob = new Blob([obfuscatedCode.textContent], { type: 'application/javascript' });
    ...
    // Client-side obfuscation function
    async function yoedzHardObfuscate(code) {
        // Load the obfuscator library dynamically
